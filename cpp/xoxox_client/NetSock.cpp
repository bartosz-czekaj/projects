...
NetSock::GetStrIP()
{
    ...
    strncpy_s(this->str_ip, inet_ntoa(*(in_addr*)&this->ip), 16);
    this->str_ip[15] = '\0';
    return this->str_ip;
}
...
NetSock::GetStrBindIP()
{
    ...
    strncpy_s(this->str_bindip, inet_ntoa(*(in_addr*)&this->bindip), 16);
...
NetSock::ReadUDP(void *buffer, int size, char *srchost, unsigned short *srcport)
{
    ...
    if (ret > 0)
    {
        if (srchost)
        {
            strcpy_s(srchost, 16, inet_ntoa(srcaddr.sin_addr)); // htonl?
            srchost[15] = '\0';
        }
